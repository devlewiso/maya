...
import { mamLessons } from "../content/mamLeccion";
...
export default function MamLeccionPage() {
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {selectedLesson.content.vocabulary.map((item: VocabularyItem, index: number) => (
              <div key={index} className="bg-white dark:bg-gray-700 p-4 rounded-lg shadow-sm hover:shadow-md transition-shadow">
                <p className="text-lg font-bold text-orange-600 dark:text-orange-400">{item.mam}</p>
                ...
                <div className="space-y-3">
                  {exercise.items.map((item: ExerciseItem, itemIndex: number) => (
                    <div key={itemIndex} className="p-3 bg-gray-50 dark:bg-gray-600 rounded-lg">
                      ...
                      {item.verb && (
                        <p className="font-medium text-gray-800 dark:text-white">Verbo: {item.verb}</p>
                      )}
                      {item.person && (
                        <p className="font-medium text-gray-800 dark:text-white">Persona: {item.person}</p>
                      )}
                      {item.role && (
                        <p className="font-medium text-gray-800 dark:text-white">Rol: {item.role}</p>
                      )}
                      {item.text && (
                      ...
                      {item.answer && (
                        <details className="mt-2">
                          <summary className="text-orange-600 dark:text-orange-400 cursor-pointer">Ver respuesta</summary>
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-orange-50 to-yellow-50 dark:from-gray-900 dark:to-gray-800">
      ...
      <section className="relative py-16 md:py-24">
        <div className="absolute inset-0 z-0 bg-orange-600 opacity-10"></div>
        <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center">
            <div className="inline-block p-2 bg-orange-100 dark:bg-orange-900 rounded-full mb-4">
              <div className="w-16 h-16 flex items-center justify-center rounded-full bg-orange-600 text-white">
            ...
            <p className="text-xl text-gray-600 dark:text-gray-300 max-w-3xl mx-auto">
              Explora nuestras lecciones interactivas para aprender Mam, uno de los idiomas mayas más importantes de Guatemala con más de 500,000 hablantes, principalmente en el occidente del país.
            </p>
          </div>
        </div>
      </section>
      ...
      <section className="py-12 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {mamLessons.map((lesson) => (
            <div 
              key={lesson.id}
              className={`bg-white dark:bg-gray-700 rounded-xl shadow-md overflow-hidden hover:shadow-lg transition-shadow duration-300 cursor-pointer ${selectedLesson?.id === lesson.id ? 'ring-2 ring-orange-500' : ''}`}
              onClick={() => handleLessonSelect(lesson)}
            >
              <div className="h-3 bg-orange-600"></div>
              <div className="p-6">
                <div className="flex items-center mb-2">
                  <span className="w-8 h-8 flex items-center justify-center rounded-full bg-orange-100 dark:bg-orange-800 text-orange-600 dark:text-orange-400 font-bold text-sm mr-3">
                    {lesson.id}
                  </span>
                ...
                <Link href={`#lesson-${lesson.id}`} className="w-full py-2 bg-orange-50 dark:bg-orange-900 text-orange-600 dark:text-orange-400 rounded-lg hover:bg-orange-100 dark:hover:bg-orange-800 transition duration-300 text-sm font-medium inline-block text-center">
                  Ver lección
                </Link>
              </div>
            </div>
      ...
      <section className="py-12 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="bg-white dark:bg-gray-700 rounded-xl shadow-md overflow-hidden">
          <div className="md:flex">
            <div className="md:flex-shrink-0 flex items-center justify-center p-6 bg-orange-50 dark:bg-orange-900">
              <svg xmlns="http://www.w3.org/2000/svg" className="h-16 w-16 text-orange-600 dark:text-orange-400" fill="none" viewBox="0 0 24 24" stroke="currentColor">
            ...
            <div className="p-6">
              ...
              <p className="text-gray-600 dark:text-gray-300 mb-4">
                El Mam tiene sonidos únicos que no existen en español. Escucha nuestras grabaciones de audio para mejorar tu pronunciación y acento.
              </p>
              <a href="#" className="inline-flex items-center text-orange-600 dark:text-orange-400 font-medium">
